...
import { MEMORY_ASSETS } from '@/features/memories/constants';

import { MemoryActionBar } from './MemoryActionBar';
import { MemoryChip } from './MemoryChip';
import { MemoryDetailsForm } from './MemoryDetailsForm';
import { MemoryEmptyState } from './MemoryEmptyState';
import { MemoryStepIndicator } from './MemoryStepIndicator';
import { MemoryTypeCard } from './MemoryTypeCard';

jest.mock('expo-image', () => ({ Image: 'ExpoImage' }));

describe('memories controls', () => {
  it('exposes chip selection and invokes its callback', async () => {
    const onPress = jest.fn();
    const { getByRole } = await render(
      <MemoryChip label="Sắp tới" selected onPress={onPress} />,
    );
  ...
  it('announces the current create step', async () => {
    const { getByRole } = await render(<MemoryStepIndicator currentStep={2} />);
  ...
  it('offers a stable recovery action for empty filters', async () => {
    ...
    const { getByRole } = await render(
      <MemoryEmptyState onClearFilter={onClear} />,
    );
