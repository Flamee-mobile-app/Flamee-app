...
import type { TimelineReminderValues } from '@/features/timeline/schemas/timelineSchema';
...
const disabledReminder: TimelineReminderValues = { enabled: false };

describe('CreateTimelineScreen', () => {
  it('uses the SVG add-timeline copy and image-upload affordance', async () => {
    const { getByText } = await render(
      <CreateTimelineScreen
        draft={{ type: 'custom', recurrence: 'none' }}
        errors={{}}
        onBack={jest.fn()}
    ...
    expect(getByText('Thêm kỉ niệm mới')).toBeTruthy();
    expect(getByText('Thêm ảnh/ Video')).toBeTruthy();
    expect(getByText('Mô tả kỉ niệm')).toBeTruthy();
  });
